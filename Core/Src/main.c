...
int main(void)
{
  ...
  while (1)
  {
	...
	HAL_IWDG_Refresh(&hiwdg);  // Prevent reset; call early each scan

    Inputs_Update();  // Sample inputs

    bool safety_ok = Safety_IsOK();  // Evaluate safety

    StateMachine_Update(safety_ok);  // Update state machine

    SequenceEngine_Update();  // Update sequence if running
